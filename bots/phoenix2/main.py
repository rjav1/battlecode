...
class Player:
    def __init__(self):
        ...
        self._returning = False          # True = walking back to core after far harvest
    ...
    def _builder(self, c):
        ...
        if self.stuck > 12:
            self.target = None
            self._returning = False  # abort return if stuck
            self.stuck = 0
            self.explore_idx += 1
        ...
        if c.get_action_cooldown() == 0:
            ...
            if ore is not None:
                ...
                if ti >= c.get_harvester_cost()[0] + 5:
                    ...
                    # Return to core after far harvest to limit chain length
                    # Tight maps: never return (too small)
                    # Balanced: >15 tiles (r^2=225). Expand: >12 tiles (r^2=144).
                    if self.core_pos and map_mode != "tight":
                        ret_dist = 144 if map_mode == "expand" else 225
                        if ore.distance_squared(self.core_pos) > ret_dist:
        ...
        # Return-to-core mode: walk back on existing conveyors, pick up missed ore
        if self._returning and self.core_pos:
            if pos.distance_squared(self.core_pos) <= 8:
                # Near core — done returning, sweep next sector
                self._returning = False
                self.explore_idx += 1
            elif not self.target:
                # Walk toward core (movement only, on existing conveyors)
                self._walk_to(c, pos, self.core_pos)
                return

        if self.target:
            self._nav(c, pos, self.target, passable)
        elif self._returning and self.core_pos:
            # Returning but no ore target — keep walking toward core
            self._walk_to(c, pos, self.core_pos)
        else:
