        ...
        if (self._sentinel_harv and self.core_pos):
            ...
            if not enemy_dir:
                ...
                if self._sentinel_step == 0:
                    ...
                    if ti >= cc + 20:
                        perp_dirs = [_perp_left(enemy_dir), _perp_right(enemy_dir),
                                     enemy_dir, enemy_dir.rotate_left(), enemy_dir.rotate_right()]
                        for pd in perp_dirs:
                            conv_pos = harv_pos.add(pd)
                            if (conv_pos.distance_squared(pos) <= 2
                                    and c.can_build_conveyor(conv_pos, pd)):
                                c.build_conveyor(conv_pos, pd)
                    ...
                    if ti >= sc + 10:
                        sent_pos = self._sentinel_conv_pos.add(self._sentinel_conv_dir)
                        for fd in [enemy_dir] + DIRS:
                            if (sent_pos.distance_squared(pos) <= 2
                                    and c.can_build_sentinel(sent_pos, fd)):
                                c.build_sentinel(sent_pos, fd)
                                self._sentinel_harv = None
                                self._sentinel_step = 0
                                return
                        self._sentinel_harv = None
        ...
        if c.get_action_cooldown() == 0:
            ...
            if ore is not None:
                ...
                if ti >= c.get_harvester_cost()[0] + 5:
                    ...
                    # Trigger sentinel auto-split on 2nd harvester, once per builder
