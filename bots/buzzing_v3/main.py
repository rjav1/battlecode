...
class Player:
    def __init__(self):
        ...
        self._last_explore_sector = 0   # sector index chosen last explore (for harvest broadcast)
        ...
        if c.get_action_cooldown() == 0:
            ...
            if ore is not None:
                ...
                if ti >= c.get_harvester_cost()[0] + 5:
                    ...
                    # Broadcast sector harvest: bitmask marker so other builders avoid this sector
    ...
    def _explore(self, c, pos, passable, rnd=0):
        ...
        # Scan nearby allied harvest-broadcast markers to find claimed sectors
        claimed_sectors = set()
        for eid in c.get_nearby_buildings():
            try:
                if (c.get_entity_type(eid) == EntityType.MARKER
                    ...
                    if val & 0x80000000:  # harvest broadcast (high bit set)
                        for sec in range(8):
                            if val & (1 << sec):
                ...
                pass
        n = len(DIRS)
        candidates = [i for i in range(n) if i not in claimed_sectors] or list(range(n))
        ...
        if area >= 1600:
            ...
            sector = candidates[(mid * 7 + self.explore_idx * 3 + rnd // 100) % len(candidates)]
            d = DIRS[sector]
            dx, dy = d.delta()
            if self.core_pos:
                cx, cy = self.core_pos.x, self.core_pos.y
            ...
            sector = candidates[(mid * 7 + self.explore_idx * 3 + rnd // 50) % len(candidates)]
            d = DIRS[sector]
            dx, dy = d.delta()
            if self.core_pos:
                cx, cy = self.core_pos.x, self.core_pos.y
            ...
            idx = candidates[(mid * 3 + self.explore_idx + rnd // 50) % len(candidates)]
            sector = idx
            d = DIRS[idx]
            dx, dy = d.delta()
            reach = max(w, h)
            far = Position(pos.x + dx * reach, pos.y + dy * reach)
        self._last_explore_sector = sector  # track for harvest broadcast
